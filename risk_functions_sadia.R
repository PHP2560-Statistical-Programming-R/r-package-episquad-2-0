...
risk <- function (data, measure = c("RR", "RD"), 
                  ci, estimate = c("crude", "summary")){
  
  d<-my.table(data) # my.table function will convert the data vector in a 2X2 table
  dt<- paste(deparse(data)) #to show data in vector format in the output
  z<-1-((100-ci)/100)/2
  
 
if (estimate == "crude"){ #will calculate the crude statistics
  if (length(d) > 1)
...
#saving the data, risk and CI for output 
  risk<-rbind(Data=dt, round(r,4))
  rownames(risk)[2] <- paste(estimate, measure, sep = " ") #paste RR or RD as the rowname
  ci<-cbind(lower.ci, upper.ci)
...
} else { #will calculate the summary statistics for stratified data
  
  
  wt.st<-c() #vector that will contain the weight of each stratum
  r.st <- c() #vector that will contain the risk (RR/RD) of each stratum
...
#saving the result of summary risk with ci
  upper.ci<- round(x + (qnorm(z) * sqrt(var.x)),4)
  lower.ci<- round(x - (qnorm(z) * sqrt(var.x)), 4)
  risk<-rbind(Data=dt, round(r,4))
  rownames(risk)[2] <- paste(estimate, measure, sep = " ")
  ci <- cbind(lower.ci, upper.ci)
...
#summary hypothesis testing:
  ...
  for (i in 1:length(d)){ #this loop will calculate the stratum-specific estimates that we need for variance
    x.t [[i]] <- a
    ex.x.t[[i]] <-  ((m1*n1)/t) 
  ...
  z.sq<-round(((sum(x)- sum(ex.x.t))^2 / sum(var.x.t)), 2)
  #add df
...
test.data<-c(-1,1,2,3,4,5,6,7)  
crude.data <- c(1,2,3,4)

risk(crude.data, measure = "RR", ci=95, estimate= "crude")
#risk(crude.data, measure = "RD", ci=95, estimate= "crude")
#risk(test.data, measure = "RR", ci=95, estimate= "summary")
#risk(test.data, measure = "RD", ci=95, estimate= "summary")

#added warning for crude data violation
#added warning for negative values
#added warning for chi square test assumption violation
